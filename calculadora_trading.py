def calculadora_trading():
    ...
    try:
        ...
        print("Error: Ingresaste un valor no numérico. Por favor, intenta de nuevo.")
        return
    ...
    while True:
        ...
        if tipo_posicion == 'long':
            ...
            print(f"Precio objetivo para cobertura (subida de {porcentaje_movimiento*100:.6f}%): {precio_objetivo:.6f}")
        else:  # Short
            precio_objetivo = precio_inicial * (1 - porcentaje_movimiento)
            print(f"Precio objetivo para cobertura (bajada de {porcentaje_movimiento*100:.6f}%): {precio_objetivo:.6f}")
        
        # Datos de la cobertura
        tokens_cobertura = tokens_iniciales * (1 + incremento_tokens)
        print(f"Cantidad de tokens para la cobertura: {tokens_cobertura:.6f}")
        ...
        if cobertura_activada:
            ...
            print(f"PNL parcial obtenido (tomado {pnl_parcial_porcentaje*100:.6f}%): {pnl_parcial:.6f}")
            print(f"PNL total acumulado: {pnl_total:.6f}")
            ...
            print(f"Nueva cantidad de tokens acumulados: {tokens_iniciales:.6f}")
        else:
            print("La cobertura no se activa en este ciclo.")

    print("\nEstrategia finalizada. ¡Éxitos en tu trading!")

# Asegurarse de que se ejecute correctamente cuando se llame desde la terminal
